...
type RoleQuery struct {
	...
	withTenantMembers *TenantMemberQuery
...
// QueryTenantMembers chains the current query on the "tenant_members" edge.
func (_q *RoleQuery) QueryTenantMembers() *TenantMemberQuery {
...
func (_q *RoleQuery) QueryPermissions() *PermissionQuery {
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withTenantMembers: _q.withTenantMembers.Clone(),
...
// WithTenantMembers tells the query-builder to eager-load the nodes that are connected to
// the "tenant_members" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithTenantMembers(opts ...func(*TenantMemberQuery)) *RoleQuery {
	query := (&TenantMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTenantMembers = query
	return _q
}
...
func (_q *RoleQuery) WithPermissions(opts ...func(*PermissionQuery)) *RoleQuery {
	query := (&PermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withTenantMembers; query != nil {
		if err := _q.loadTenantMembers(ctx, query, nodes,
			func(n *Role) { n.Edges.TenantMembers = []*TenantMember{} },
			func(n *Role, e *TenantMember) {
				n.Edges.TenantMembers = append(n.Edges.TenantMembers, e)
...
func (_q *RoleQuery) loadTenantMembers(ctx context.Context, query *TenantMemberQuery, nodes []*Role, init func(*Role), assign func(*Role, *TenantMember)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantmember.FieldRoleID)
	}
	query.Where(predicate.TenantMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.TenantMembersColumn), fks...))
	}))
...
func (_q *RoleQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *Permission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(permission.FieldRoleID)
	}
	query.Where(predicate.Permission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.PermissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
