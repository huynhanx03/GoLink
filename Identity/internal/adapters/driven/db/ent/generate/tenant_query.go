...
func (_q *TenantQuery) QueryTenantMembers() *TenantMemberQuery {
...
func (_q *TenantQuery) WithTenantMembers(opts ...func(*TenantMemberQuery)) *TenantQuery {
	query := (&TenantMemberClient{config: _q.config}).Query()
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withTenantMembers; query != nil {
		if err := _q.loadTenantMembers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.TenantMembers = []*TenantMember{} },
			func(n *Tenant, e *TenantMember) {
	...
	if query := _q.withDomains; query != nil {
		if err := _q.loadDomains(ctx, query, nodes,
			...
			func(n *Tenant, e *Domain) {
				n.Edges.Domains = append(n.Edges.Domains, e)
				if !e.Edges.loadedTypes[0] {
					e.Edges.Tenant = n
				}
			}); err != nil {
...
func (_q *TenantQuery) loadTenantMembers(ctx context.Context, query *TenantMemberQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantMember)) error {
	...
	query.Where(predicate.TenantMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.TenantMembersColumn), fks...))
	}))
