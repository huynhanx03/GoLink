...
type Role struct {
	...
	// Parent role ID for hierarchy (root = -1)
	ParentID int `json:"parent_id,omitempty"`
	// Nested Set left value for hierarchy
	Lft int `json:"lft,omitempty"`
	// Nested Set right value for hierarchy
...
func (e RoleEdges) TenantMembersOrErr() ([]*TenantMember, error) {
	if e.loadedTypes[0] {
...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case role.FieldID, role.FieldDeletedBy, role.FieldLevel, role.FieldParentID, role.FieldLft, role.FieldRgt:
...
func (_m *Role) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case role.FieldLft:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field lft", values[i])
			} else if value.Valid {
				_m.Lft = int(value.Int64)
			}
		case role.FieldRgt:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field rgt", values[i])
			} else if value.Valid {
				_m.Rgt = int(value.Int64)
			}
		default:
...
// QueryTenantMembers queries the "tenant_members" edge of the Role entity.
func (_m *Role) QueryTenantMembers() *TenantMemberQuery {
	return NewRoleClient(_m.config).QueryTenantMembers(_m)
}
...
func (_m *Role) Unwrap() *Role {
	...
	if !ok {
		panic("generate: Role is not a transactional entity")
	}
...
func (_m *Role) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("lft=")
	builder.WriteString(fmt.Sprintf("%v", _m.Lft))
	builder.WriteString(", ")
	builder.WriteString("rgt=")
	builder.WriteString(fmt.Sprintf("%v", _m.Rgt))
	builder.WriteByte(')')
	return builder.String()
}

func (_m *Role) GetID() int {
	return _m.ID
}

func (_m *Role) SetID(id int) {
	_m.ID = id
}
