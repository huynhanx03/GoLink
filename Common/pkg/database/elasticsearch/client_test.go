...
func testCreateBatch(t *testing.T, ctx context.Context, repo *BaseRepository[TestDocument, string]) {
	...
	// repo.CreateBatch takes []*T. T=TestDocument. So []*(TestDocument).
	docs := []*TestDocument{doc1, doc2}

	if err := repo.CreateBatch(ctx, docs); err != nil {
...
func testDeleteBatch(t *testing.T, ctx context.Context, repo *BaseRepository[TestDocument, string]) {
	...
	if err := repo.DeleteBatch(ctx, []string{"8", "9"}); err != nil {
